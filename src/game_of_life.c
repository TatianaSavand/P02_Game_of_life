...
int main() {
  int **current = malloc(Y * X * sizeof(int) + Y * sizeof(int *)),
      **next = malloc(Y * X * sizeof(int) + Y * sizeof(int *));
  int *prtCurrent = (int *)(current + Y), *prtNext = (int *)(next + Y);

  for (int i = 0; i < Y; ++i)
    current[i] = prtCurrent + X * i;
  for (int i = 0; i < Y; ++i)
    next[i] = prtNext + X * i;
